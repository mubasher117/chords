...
def fit(train_percentage, fold=5):
    ...
    for folds in range(fold):
        ...
        for class_counter in range(10):
            # create an index of size 100
            stack = list(range(start, start+100))
            for song_counter in range(int(train_percentage)):
                # randomly choose numbers from index
                index = random.choice(stack)
                stack.remove(index)  # remove the choosen number from index
                # select songs from that index for training
                random_song = Xall[index]
                Xindex.append(index)
                if flag:
...
def getprob(filename):
    ...
    dd = dict(
        zip(['Classical', 'Pop', 'Jass', 'Metal', 'Hipop', 'Rock'], prob))
    print(prob)
...
def random_cross_validation(train_percentage, fold):
    ...
    print("Training accuracy with %d fold %f: " %
          (int(fold), resTrain / int(fold)))
    print("Testing accuracy with %d fold %f: " %
          (int(fold), resTest / int(fold)))
